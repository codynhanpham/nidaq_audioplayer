...
async def handle_seek(websocket, data: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        max_duration = load_audio.nidaq_player._audio_duration if hasattr(load_audio.nidaq_player, '_audio_duration') else 0
        is_valid, error_msg = validate_numeric_range(position, position_field, min_val=0, max_val=max_duration)
        if not is_valid:
            return create_error_response(error_msg)
        
        # Perform seek
        old_status = load_audio.nidaq_player.get_status()
        old_position = old_status.get('current_time', 0)
        
        load_audio.nidaq_player.seek(position)
        
        new_status = load_audio.nidaq_player.get_status()
