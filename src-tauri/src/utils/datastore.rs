...
pub async fn save_audio_metadata(
    audio_metadata: Vec<metadata::AudioMetadata>,
    output_path: String,
    ...
    encoder
        .write_all(&data)
        .map_err(|e| format!("Failed to compress metadata: {e:?}"))?;
    let compressed_data = encoder
        .finish()
        .map_err(|e| format!("Failed to finish compression: {e:?}"))?;

    std::io::Write::write_all(&mut writer, &compressed_data)
...
pub async fn load_audio_metadata(
    input_path: String,
) -> Result<Vec<metadata::AudioMetadata>, String> {
    let input_path = PathBuf::from(input_path);

    let mut reader = Vec::new();
    let mut decoder = ZlibDecoder::new(&mut reader);
    decoder
        .write_all(&std::fs::read(&input_path).map_err(|e| format!("Failed to read file: {e:?}"))?)
    ...
    let audio_metadata: Vec<metadata::AudioMetadata> =
        match bincode::decode_from_slice(&reader, bincode::config::standard()) {
            Ok((data, _)) => data,
