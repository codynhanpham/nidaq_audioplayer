...
<script lang="ts">
	...
	function seekToChapter(timestamp: number) {
		...
		const progress = (timestamp / MediaPlayerData.duration) * 100;
		// Make sure to clamp the progress value between 0 and 100
		MediaPlayerData.progress = Math.max(0, Math.min(progress, 100));
	}
	...
	function togglePlayPause() {
		if (MediaPlayerData.isPlaying) {
			...
			MediaPlayerData.isPlaying = true;
		}
	}
    ...
    <div class="group/player relative w-full h-full flex items-center justify-between px-1.5 sm:px-3 pt-1">
		...
		<div class="w-full h-full flex flex-col items-center justify-center">
			...
			<div class="w-full flex-1 flex items-center justify-between gap-8 md:gap-10">
				...
				<section data-player-media-info class="flex flex-1 grow items-center justify-center gap-5 sm:gap-6">
					{#if MediaPlayerData.audioInfo}
						<div class="[&_div]:!rounded-sm [&_span]:!rounded-sm">
							{#if MediaPlayerData.audioInfo?.thumbnail}
								<Avatar.Root class="size-8 sm:size-9">
									<Avatar.Image class="h-full aspect-square" src="https://github.com/shadcn.png" alt="@shadcn" />
						...
						<div class="max-w-full flex flex-col items-center justify-center gap-0">
							<span class="mb-0 text-center text-sm text sm:text-base max-w-full text-ellipsis line-clamp-1" title={MediaPlayerData.audioInfo?.name || ""}>{MediaPlayerData.audioInfo?.name || ""}</span>
				...
				<section data-player-options class="flex items-center justify-center gap-0.5">
					<div class="hidden sm:block">
						<Select.Root type="single" bind:value={currentChapter} disabled={!MediaPlayerData.audioInfo?.chapters || MediaPlayerData.audioInfo.chapters.length === 0}>
							...
							<Select.Content class="z-[99999] bg-background/50 backdrop-blur-xl" side="top" sideOffset={16} collisionPadding={{top: 42, right: 4}}>
								<Select.SelectGroup>
									<Select.Label class="py-1 px-1.5">Chapters</Select.Label>
									{#if MediaPlayerData.audioInfo?.chapters && MediaPlayerData.audioInfo?.chapters.length > 0}
										{#each MediaPlayerData.audioInfo.chapters as chapter, i}
					...
					<div>
						{#if MediaPlayerData.muted}
							<Button variant="ghost" class="media-button" onclick={() => {}} aria-label="Unmute">
								<VolumeX class="size-4" />
							</Button>
						{:else if MediaPlayerData.volume >= 0.60}
							<Button variant="ghost" class="media-button" onclick={() => {}} aria-label="Mute">
